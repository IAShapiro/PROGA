...
public:
	...
	Matrix(Matrix const& source) {//конструктор копировани€
		size1 = source.size1;
		size2 = source.size2;
		array = new int*[size1];
		for (int i = 0; i < size1; i++)
	...
	Matrix(Matrix&& source) {//конструктор перемещени€
		size1 = source.size1;
		size2 = source.size2;
		array = source.array;
	...
	const Matrix operator= (Matrix& r)
	{
		...
		delete[] array;//ѕравильное ли в данном случае предварительное очищение места в пам€ти?..
	...
	friend Matrix operator*(const Matrix& r1, const Matrix& r2)
	{
		...
		{
			std::cout << "ћатрицы не могут быть умножены!\n";
			exit(1);
		}
